...
// .wrangler/tmp/bundle-1LXx2g/strip-cf-connecting-ip-header.js
...
async function handleBalances(url, env) {
  ...
  try {
    ...
    const result = Object.values(balances).filter((t) => t.balance > 0).map((t) => ({
      symbol: t.symbol,
      name: t.name,
...
// worker/handlers/native-balance.js
async function handleNativeBalance(url, env) {
  const address = url.searchParams.get("address") ?? "";
  const chain = url.searchParams.get("chain") ?? "ethereum";
  if (!isAddress(address))
    return fail("address \u304C\u4E0D\u6B63\u3067\u3059");
...
__name(handleNativeBalance, "handleNativeBalance");
...
var worker_default = {
  async fetch(request, env) {
    ...
    switch (url.pathname) {
      ...
      case "/api/native-balance":
        return handleNativeBalance(url, env);
    }
    if (env.ASSETS) {
...
// .wrangler/tmp/bundle-1LXx2g/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1LXx2g/middleware-loader.entry.ts
