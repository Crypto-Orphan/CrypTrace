// graph.js — Node/Edge・グラフ描画・物理シミュレーション・CEX/DEX判定

async function getExchangeName(address, chainId) {
    const lowerAddr = address.toLowerCase();

    // ステップ1: ハードコーディングされた名前をチェック（即座）
    if (EXCHANGE_NAMES[lowerAddr]) {
        return EXCHANGE_NAMES[lowerAddr];
    }

    // ステップ2: Workers経由でコントラクト情報を取得
    if (chainId) {
        try {
            const chainKey = Object.keys(CHAIN_CONFIGS).find(k => CHAIN_CONFIGS[k].chainId === chainId) || 'ethereum';
            const result = await API.getContract(address, chainKey);
            if (result && result.isCexDex && result.label) {
                return result.label;
            }
            ...
            console.warn('getExchangeName API error:', e);
        }
    }
