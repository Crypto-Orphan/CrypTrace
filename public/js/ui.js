// ui.js — UI・イベント・ツールチップ・パネル・トークン操作
...
document.getElementById('exploreBtn').addEventListener('click', async () => {
    ...
    if (!API_KEY || API_KEY === 'YOUR_ETHERSCAN_API_KEY_HERE') {
        ...
        showError('APIキーエラー', 'APIキーを設定してください', 
            'コード内の API_KEY 変数に有効なEtherscan APIキーを設定してください。');
        return;
    }
    ...
    try {
        ...
        const transactions = await getTransactions(address, chainId, limit, tokenType, tokenAddress);
...
async function showInfoPanel(node) {
    ...
    if (!exchangeName && node.isCexDex) {
        exchangeName = await getContract(node.address, node.chainId).then(r => r?.label || null);
    }
...
async function showAddressDetails(node) {
    ...
    // CEX/DEX名を取得（非同期）
    const exchangeName = await getContract(node.address, node.chainId).then(r => r?.label || null);
    ...
    // トークン詳細を取得（実際の残高付き）
    const tokens = await getBalances(node.address, node.chainId);
...
async function runTokenSearch() {
    ...
    try {
        if (searchMethod === 'address') {
            ...
            const token = await searchToken(searchInput, chainId, "address").then(r => r[0]);
            showTokenResult(token, infoDisplay);
        } else {
            const tokens = await searchToken(searchInput, chainId, "symbol");
            if (tokens.length === 1) {
...
console.log('CoinGecko API Key configured:', COINGECKO_API_KEY ? 'Yes' : 'No');
